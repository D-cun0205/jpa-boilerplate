...
```
build.gradle 참조, spring boot 3.x, java 17 버전 기준이며
spring boot 2.x 버전에 대해서는 검색 필요
```

### 별칭

```
같은 테이블에 대해서 조인이 필요한 경우
ex: QMember member = new QMember("m1"), QMember member = new QMember("m2")
위 와 같이 설정하여 조인하면 되고 일반적인 케이스에서는 이미 생성되어 있는 생성자 호출 방법을 사용하는 것을 권장
```
...
### 사용자 정의 레파지토리, 클래스와 인터페이스를 보여주며 설명 

```
1. Interface, class(구현체), 구현체를 JpaRepository 를 사용하는 클래스에 상속
2. 특화된 기능을 가진 별도의 Repository 생성 후 사용
```
